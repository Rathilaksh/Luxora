...
// GET all listings (with host info, reviews, amenities)
...
app.get('/api/listings/:id', async (req, res) => {
  try {
    ...
    const listing = await prisma.listing.findUnique({
      where: { id },
      include: { 
        host: { select: { id: true, name: true, email: true, avatar: true, bio: true, phone: true } },
        bookings: true,
        reviews: {
...
app.post('/api/bookings', authMiddleware, async (req, res) => {
  try {
    ...
    const conflict = await prisma.booking.findFirst({
      ...
      // Create listing (host-only placeholder: any authenticated user acts as host)
